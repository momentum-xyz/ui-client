...
/**
 * Taken from https://github.com/TalismanSociety/talisman-web3react-example/blob/main/talisman/TalismanConnector.ts
 * as recommended in https://docs.talisman.xyz/talisman/build-the-paraverse/developer-resources/evm-integration-for-dapps
 */
...
const talismanWindow =
  typeof window !== 'undefined' ? (window as TalismanWindow) : ({} as TalismanWindow);
...
export class TalismanConnector extends AbstractConnector {
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId;
    try {
      chainId = await (talismanWindow.talismanEth.send as Send)('eth_chainId').then(
        parseSendReturn
      );
    } catch {
    ...
    if (!chainId) {
      try {
        chainId = await (talismanWindow.talismanEth.send as Send)('net_version').then(
          parseSendReturn
        );
      } catch {
