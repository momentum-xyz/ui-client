...
export class WhispControllable extends Whisp {
  ...
  private static readonly CAMERA_SHAKE_MAGNITUDE = 0.0012;
  private static readonly CAMERA_SHAKE_SPRING = 0.004;
  private static readonly CAMERA_SHAKE_POWER = 1.2;
  private static readonly CAMERA_SPRING = 0.004;
  private static readonly ACCELERATION = 50;
  private static readonly ACCELERATION_SPRINT = 120;
  private static readonly FRICTION = 0.14;
  private static readonly LOOK_SENSITIVITY = 0.002;
  ...
  private createParticlesStars(scene: Scene) {
    ...
    const makeParticleSpawn = (particlePosition: Vector3, initial = false) => {
      ...
      const distance = initial
        ? Math.random() *
            (WhispControllable.STAR_SPAWN_DISTANCE + WhispControllable.STAR_CLIP_DISTANCE) -
          WhispControllable.STAR_SPAWN_DISTANCE
        : WhispControllable.STAR_SPAWN_DISTANCE +
          (WhispControllable.STAR_CLIP_DISTANCE - WhispControllable.STAR_SPAWN_DISTANCE) *
            Math.random();
    ...
    this.particlesStars.addShape(
      MeshBuilder.CreatePlane('ParticlesStarsPlane', {}, scene),
      WhispControllable.STAR_COUNT
    );
    ...
    this.particlesStars.updateParticle = function (particle) {
      ...
      if (
        dot < -WhispControllable.STAR_SPAWN_DISTANCE ||
        ...
        const visibility = Math.max(
          0,
          Math.cos(dot * ((Math.PI * 0.5) / WhispControllable.STAR_SPAWN_DISTANCE))
        );
        ...
        particle.position.x -= velocity.x * scene.deltaTime * 0.001;
        particle.position.y -= velocity.y * scene.deltaTime * 0.001;
        particle.position.z -= velocity.z * scene.deltaTime * 0.001;
        particle.rotation.set(directionAngles.x, directionAngles.y, directionAngles.z);
      }
  ...
  private updateCamera(delta: number) {
    ...
    this.cameraPosition.x =
      this.position.x +
      Math.cos(Math.PI - this.camera.beta) *
        WhispControllable.CAMERA_RAISE *
        Math.cos(this.camera.alpha);
    this.cameraPosition.y =
      this.position.y + Math.sin(Math.PI - this.camera.beta) * WhispControllable.CAMERA_RAISE;
    ...
    this.sphere.rotation.set(this.animationPhase * Math.PI * -2, -this.node.rotation.y, 0);
  }
  ...
  update(delta: number) {
    ...
    const acceleration =
      this.inputState & WhispControllable.BIT_SPRINT
